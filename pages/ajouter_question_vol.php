...
// Récupérer les questions pour la page actuelle
...
if ($filter_formation > 0) {
    $sql_questions = "SELECT q.id, q.question_text, f.title, f.id as formation_id 
                     FROM questions q 
                     JOIN formations f ON q.formation_id = f.id 
                     WHERE f.id = $filter_formation 
                     ORDER BY q.id DESC 
                     LIMIT $offset, $questions_per_page";
    $result_questions = $conn->query($sql_questions);
    
    // Recalculer la pagination
    $count_query = "SELECT COUNT(*) as total FROM questions WHERE formation_id = $filter_formation";
    $count_result = $conn->query($count_query);
    $total_questions = $count_result->fetch_assoc()['total'];
    $total_pages = ceil($total_questions / $questions_per_page);
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['add_question'])) {
    ...
    if (empty($formation_id) || empty($question_text) || count($answers) != 4 || $correct_answer < 0 || $correct_answer > 3) {
        ...
        if (!$all_answers_filled) {
            ...
            header("Location: ajouter_question_vol.php?success=1");
            exit();
        }
    }
}
...
if (isset($_GET['delete']) && is_numeric($_GET['delete'])) {
    ...
    if ($delete_answers_result && $delete_question_result) {
        ...
        header("Location: ajouter_question_vol.php?deleted=1&nocache=" . time());
        exit();
    } else {
        ...
        header("Location: ajouter_question_vol.php?error=" . urlencode($error_message) . "&nocache=" . time());
        exit();
    }
}
...
if (isset($_POST['delete_all']) && $_POST['delete_all'] == 'confirm') {
    ...
    // Rediriger vers la même page avec un message de succès
    header("Location: ajouter_question_vol.php?deleted_all=1");
    exit();
}
...
<head>
    ...
    <title>Gestion des Questions - TTA</title>
    ...
    <div class="main-content">
        <div class="page-header">
            ...
            <ul class="breadcrumb">
                ...
                <li class="breadcrumb-item">Gestion des questions</li>
            </ul>
        </div>
        ...
        <?php if (isset($_GET['deleted_all'])): ?>
            <script>
                document.addEventListener('DOMContentLoaded', function() {
                    showSuccess('Succès', 'Toutes les questions ont été supprimées.');
                });
            </script>
        <?php endif; ?>
        ...
        <div class="card animate-on-scroll">
            <div class="card-header">
                <h2 class="card-title">Ajouter une question</h2>
            </div>
            <div class="card-body">
        ...
        <div class="card animate-on-scroll">
            <div class="card-header">
                ...
                <button type="button" class="btn btn-sm btn-danger" onclick="confirmDeleteAll()">
                    <i class="fas fa-trash"></i> Supprimer toutes les questions
                </button>
            </div>
            <div class="card-body">
                <!-- Filter Form -->
                <form method="get" class="filter-form">
                    ...
                    <?php if ($filter_formation > 0): ?>
                        <a href="ajouter_question.php" class="btn btn-outline-secondary">Réinitialiser</a>
                    <?php endif; ?>
                </form>
                                ...
                                <?php while ($question = $result_questions->fetch_assoc()): ?>
                                    <tr>
                                        ...
                                        <td class="table-actions">
                                            <a href="edit_question_vol.php?id=<?php echo $question['id']; ?>" class="btn-action btn-edit" data-tooltip="Modifier">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            <!-- Remplacer le bouton de suppression par un lien direct -->
                                            <a href="ajouter_question_vol.php?delete=<?php echo $question['id']; ?>" class="btn-action btn-delete" data-tooltip="Supprimer" onclick="return confirm('Êtes-vous sûr de vouloir supprimer cette question?');">
    ...
    <script>
        function confirmDeleteAll() {
            Swal.fire({
                ...
                text: 'Êtes-vous sûr de vouloir supprimer TOUTES les questions ? Cette action est irréversible.',
                icon: 'warning',
