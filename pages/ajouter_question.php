...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['add_question'])) {
    ...
    if (empty($formation_id) || empty($question_text) || count($answers) != 4 || $correct_answer < 0 || $correct_answer > 3) {
        ...
        if (!$all_answers_filled) {
            ...
            header("Location: ajouter_question.php?success=1");
            exit();
        }
    }
}
...
if (isset($_GET['delete']) && is_numeric($_GET['delete'])) {
    ...
    if ($delete_answers_result && $delete_question_result) {
        ...
        header("Location: ajouter_question.php?deleted=1&nocache=" . time());
        exit();
    } else {
        ...
        header("Location: ajouter_question.php?error=" . urlencode($error_message) . "&nocache=" . time());
        exit();
    }
}
...
if (isset($_POST['delete_all']) && $_POST['delete_all'] == 'confirm') {
    ...
    // Rediriger vers la même page avec un message de succès
    header("Location: ajouter_question.php?deleted_all=1");
    exit();
}
                                ...
                                <?php while ($question = $result_questions->fetch_assoc()): ?>
                                    <tr>
                                        ...
                                        <td class="table-actions">
                                            <a href="edit_question.php?id=<?php echo $question['id']; ?>" class="btn-action btn-edit" data-tooltip="Modifier">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            <!-- Remplacer le bouton de suppression par un lien direct -->
                                            <a href="ajouter_question.php?delete=<?php echo $question['id']; ?>" class="btn-action btn-delete" data-tooltip="Supprimer" onclick="return confirm('Êtes-vous sûr de vouloir supprimer cette question?');">
